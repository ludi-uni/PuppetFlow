...
StateStore
  ...
  → executeBehavior(behavior)     ← PFScript stateful 関数（共有 StatefulStore）
  → executeMotionGraph(graph)     ← stateful ノード
  → merge（同一キーは平均）
  → Motion Modifiers
  → Extension Layer（extensions / MotionPack — runStatefulNumber 利用可）
  → Adapters
```
...
| `clamp`                                                                                 | 範囲制限                                                             |
| `sin`, `noise`                                                                          | 周期・ノイズ（無状態）                                               |
| `oscillator`, `smooth`, `spring`, `randomHold`, `blink`, `breath`, `wander`, `cooldown` | **Stateful** ノード（[stateful.md](stateful.md)）                    |
| `output`                                                                                | MotionState キーへ出力                                               |
| `motionPack`                                                                            | Extension Pack（エッジ不要・Extension Layer へ）                     |
| `motionGenerator`                                                                       | Extension Generator                                                  |
| `motionFunction`                                                                        | Extension スカラー関数（`heartbeat` 等・エッジで数値パイプラインへ） |
