...
**Current status:** Phase B、Node Host内Phase C、Studio 4A/4B、共有制御Phase 4C完了。Phase 4Dで共有CLIのVMCを単一合成senderへ接続し、実UDPでExpression/wave/clearと停止を確認した。Phase EではAITuberのshared actingを共通Control clientへ接続し、既存LipSync WebSocketを同じHostの専用Sourceへ接続した。実CLIと実Acting/LipSync sessionによるfixture playback anchorから共有state・UDPまでを確認済みだが、`/operator/message`の実音声再生と外部Viewerは未確認。Studio全設定共有、AITuberの非Acting/LipSync機能、旧standalone整理、portable自動起動も未完了。
...
| **E** | AITuber → Control transport migration    | shared `ActingTransport`を共通Control client化し、既存LipSync Sourceを同じHostへ接続。speech anchor/timingを維持 | **実装完了・実発話受入未完:** 旧supervisor/MCP childを起動せず、fixture anchorのBody/Expression/LipSyncが一つのHostへ入る。`/operator/message`実音声、旧transport整理、未共有機能は残る。 |
