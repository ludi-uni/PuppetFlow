...
| `PuppetFlowControl`                         | 現在は未存在                                                                | 外部semantic control boundary                                                                   | **NEW**                       | Studio、AITuber、AI Agent、HTTP、MCPが共有する唯一の入口として追加する。                                                                                                          |
| `PuppetFlowHost`                            | 現在は未存在。`buildRuntime`等が部分的に担う                                | Runtime、Control implementation、adapters、lifecycleの所有                                      | **NEW**                       | Runtimeを一つだけ生成・起動・停止するHostを追加する。                                                                                                                             |
...
1. Runtimeを直接生成する入口がStudio、CLI、sibling hostに分散している。
