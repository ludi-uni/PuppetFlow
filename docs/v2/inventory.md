...
| Block Editor (Blockly)                      | `apps/studio/src/scratch`, `ScratchEditor`                                  | BlocklyをBehavior ASTへ変換                                                                     | **OPTIONAL EDITOR / IMPLEMENTED**    | Phase Hで既定OFF、Simple非表示、Expertの明示設定後だけ遅延mount。実装・Preset内の関連データ・実行互換は維持し、別配布plugin化は未完。                                             |
...
## Phase G cleanup decisions

| 対象                                                                                                       | 現在の利用者                                                                                    | 代替 / 残す理由                                                          | 判断 / 削除可能になる条件                                                                 |
| ---------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------- |
| `packages/runtime/src/control.ts`とRuntime packageの`PuppetFlowControl` / `createPuppetFlowControl` export | 実行時consumerなし。残存は専用testとdeprecated文書だけ                                          | Host、Studio、HTTP、workspace MCPはcanonical `@puppetflow/control`を使用 | **削除。** v2 breaking change。旧importは`@puppetflow/control`のcamelCase DTOへ移行する。 |
| 旧Behavior HTTP                                                                                            | `apps/cli/src/commands/run.ts`が`behaviorApi`設定時に起動                                       | 通常CLI `run`の既存機能                                                  | **維持。** CLI利用者の移行または機能廃止決定後に削除可能。                                |
