...
import { collectMapperCustomParamIdsFromParts } from "./collect-mapper-custom-params.js";
...
`,
    ...
    expect(collectMapperCustomParamIdsFromParts(presetJson, extensionsJson)).toEqual([
      "MouthA",
      "earAngle",
      "tailWag",
    ]);
  ...
  it("falls back to cached behavior when PFScript fails to compile", () => {
    ...
    expect(collectMapperCustomParamIdsFromParts(presetJson, "{}")).toContain("MouthO");
  });
});
